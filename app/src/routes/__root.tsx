...
const DEFAULT_TITLE = "Kruger Vacations";
const DEFAULT_DESCRIPTION = "Explore House 225B at Kruger Park Lodge in Hazyview.";
...
function buildHead(meta: AppMeta) {
  ...
  return {
    meta: [
      ...
      { name: "author", content: "Kruger Vacations" },
      { name: "theme-color", content: scrollScrubTheme.background },
      { property: "og:url", content: "https://kruger-vacations-house-225b.higgsfield.app/" },
    ...
    links: [
      ...
      { rel: "canonical", href: "https://kruger-vacations-house-225b.higgsfield.app/" },
...
function NotFoundComponent() {
  return <main className="status-page"><h1>That room is not here</h1><p>Return to House 225B to explore the property.</p><Link to="/">Explore House 225B</Link></main>;
}
function ErrorComponent({ error, reset }: { error: Error; reset: () => void }) {
  const router = useRouter();
  useEffect(() => { reportHiggsfieldError(error, { boundary: "tanstack_root_error_component" }); }, [error]);
  return <main className="status-page"><h1>This page could not load</h1><p>Please try again, or contact info@krugervacations.co.za.</p><button onClick={() => { router.invalidate(); reset(); }}>Try again</button><a href="/">Explore House 225B</a></main>;
}
